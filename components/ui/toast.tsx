...
export function Toast({ message, type = "info", duration = 3000, onClose }: ToastProps) {
  ...
  return (
    ...
    >
      ...
      <p className="min-w-0 font-fredoka flex-1 text-lg font-black text-card-foreground">{message}</p>
      <button
        type="button"
        onClick={handleClose}
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const container =
    mounted && typeof document !== "undefined"
      ? createPortal(
          <div
            className="fixed top-4 right-4 z-[9999] flex min-w-[320px] max-w-[420px] flex-col gap-2 font-fredoka"
